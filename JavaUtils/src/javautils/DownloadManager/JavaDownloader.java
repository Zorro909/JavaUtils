...
public class JavaDownloader {
	...
	public static void downloadFile(URL url, File target, DownloadListener... dl) throws IOException {
		...
		if (dl != null && dl.length > 0)
			for (DownloadListener d : dl)
				d.startedDownloading(target.getName(), length);
		...
		while ((chunkSize = is.read(chunk)) != -1) {
			...
			if (dl != null && dl.length > 0)
				for (DownloadListener d : dl)
					d.downloadTileFile(target.getName(), chunkSize);
		}
	...
	/**
	 * Downloaded eine Datei Asynchron von der URL url in Die File target mit einem
	 * optionalem DownloadListener dl
	 *
	 * @param url    Die Url Der heruterzuladenden Datei
	 * @param target Die Datei in die geschrieben werden soll
	 * @param dl     Ein Optionaler DownloadListener (kann auch null sein)
	 */
	public static void downloadFileAsynchronously(final URL url, final File target, final DownloadListener... dl) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					while (true) {
						...
						if (concurrentDownloads != maxDownloadsAtATime) {
							try {
								downloadFile(url, download, downloadListener);
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
