...
public class TCPManager {
   ...
   * @param lookForClientsAutomatically
   *          If the Manager should automatically Look for Clients
   * @param listener
   *          An TcpServerListener [Only Needed if
  ...
  public static TcpServer startServer(
          int port,
          boolean lookForClientsAutomatically,
  ...
  public static TcpServer startServer(
    ...
    if (mode.getEncryption().equals(TcpServerMode.NO_ENCRYPTION.getEncryption())) {
      ...
      System.setProperty("javax.net.ssl.keyStore", mode.getCertificateFile().getAbsolutePath());
      System.setProperty("javax.net.ssl.keyStorePassword", mode.getCertificatePassword());
   ...
   * @param tcpServer
   * @param listener
   *          An TcpServerListener
   */
  public static void lookForClientsAutomatically(
          final TcpServer tcpServer,
          final TcpServerListener listener) {
    if (lookForClients == null) {
      lookForClients = new ArrayList<Thread>();
    }
   ...
   *
   * @param url
   *          The Url where the TCP Server is
   * @param port
   *          The Port where the Server listens
   * @param automaticListen
   *          If the Manager should autmatic Listen for incoming Messages
   *          (Recommended)
   * @param listener
   *          An TcpClientListener [Only Needed if {@code automaticListen} is
   *          true]
  ...
  public static TcpConnection connect(
          String url,
          int port,
          boolean automaticListen,
          TcpClientListener listener,
          TcpConnectionMode mode)
          throws UnknownHostException, IOException, NoSuchAlgorithmException, CertificateException,
    ...
    if (automaticListen) {
      automaticClientListen(t, listener);
    }
    return t;
  }
   ...
   *
   * @param tcp
   *          The TCP Connection for the Automatic Listening
   * @param listener
   *          An TcpClientListener
   */
