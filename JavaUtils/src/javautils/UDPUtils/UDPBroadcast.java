...
public class UDPBroadcast {
    ...
    public static void startNewBroadcastRequest(final int port, final String request,
        ...
        if (startNewThread) {
            new Thread(new Runnable() {
                ...
                @Override
                public void run() {
                    _startNewBroadcastRequest(port, request, timeout, responseListener,
                                    "255.255.255.255");
                }
    ...
    public static void startNewBroadcastRequest(final int port, final String request,
                    boolean startNewThread, final int timeout,
                    final UDPBroadcastResponseListener responseListener,
                    final String defaultBroadcastIP) {
        if (startNewThread) {
            new Thread(new Runnable() {
        ...
        try {
            ...
            try {
                DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length,
                                InetAddress.getByName(defaultBroadcastIP), port);
                c.send(sendPacket);
                System.out.println(
                                ">>> Request packet sent to: " + defaultBroadcastIP + " (DEFAULT)");
            } catch (Exception e) {
            }
		...
		try {
			...
			while (true) {
				...
				String message = new String(packet.getData()).trim();
					String r = requestListener.process(message, packet.getAddress());
